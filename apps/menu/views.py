...
from menu.models import ReservoirModel, HruModel, SubModel, LandType, RchModel
...
class IndexView(View):
    def get(self, request):
        # 水库的id
        reservoir_id = request.GET.get('id')
        # print(reservoir_id)
        reservoir = ReservoirModel.objects.filter(id=reservoir_id)
        # print(reservoir)
        reservoir_dict = json.loads(serializers.serialize(format='json', queryset=reservoir, ensure_ascii=False))
        reservoir_dict = [{'id': i["pk"], 'fields': i['fields']} for i in reservoir_dict]
        return JsonFormatUtil(data=reservoir_dict).parseJson()
